...
When(/^I create a policy called "(.*?)" that is part of a policy called "(.*?)"$/) do |policy_name, part_of_policy_name|
  stub_publishing_api
  stub_rummager

  create_policy(name: policy_name, parent_policies: [part_of_policy_name])
end
